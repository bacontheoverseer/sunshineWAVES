...
declare global {
  interface Window {
    sunshineUpdate?: { hideSuccess: (calledByOther: boolean) => void };
...
function _initSettings(): void {
  ...
  window.sunshineExportAllData = async function (): Promise<MasterExport> {
    const masterExport: MasterExport = {
      localStorage: Object.keys(localStorage).reduce(
        (acc: Record<string, string>, key: string) => {
          if (key !== "sunshine-sync-meta") {
  ...
  async function exportAllData(fileName: string): Promise<void> {
    try {
      const masterExport = await window.sunshineExportAllData!();
  ...
  window.sunshineImportDataFromObject = async function (
    data: unknown,
