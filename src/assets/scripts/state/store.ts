...
export const store = {
  ...
  setupWindowsunshineApp() {
    window.sunshineApp = window.sunshineApp || {};
    window.sunshineApp.tabs = this.tabs as any;
    window.sunshineApp.splitPair = this.splitPair;
    window.sunshineApp.isLoading = false;
    window.sunshineApp.allGames = this.allGames;
    window.sunshineApp.getActiveTab = () => this.getActiveTab();
    window.sunshineApp.renderTabs = () => this.notify();
    window.sunshineApp.resetSession = () => this.resetSession();
    window.sunshineApp.openNewTabFromServiceWorker = (url: string | null, options: { title?: string; openerTabId?: number } = {}) => {
      if (!url) return null;
      const tab = this.addTab(url, options.title || "fetching data...");
      if (tab && options.openerTabId) tab.openerTabId = options.openerTabId;
      return tab;
    };
    window.sunshineApp.handleSearch = async (query: string, gameName?: string, gameIcon?: string | null) => {
      const activeTab = this.getActiveTab();
      if (activeTab) {
        if (gameName) {
