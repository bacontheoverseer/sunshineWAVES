...
function getBookmarks(): Bookmark[] {
  try {
    const raw = localStorage.getItem("sunshine-bookmarks");
    if (!raw) {
      const defaults = DEFAULT_BOOKMARKS.slice();
      localStorage.setItem("sunshine-bookmarks", JSON.stringify(defaults));
      return defaults;
    }
...
function saveBookmarks(bookmarks: Bookmark[]) {
  localStorage.setItem("sunshine-bookmarks", JSON.stringify(bookmarks));
}
...
export default function Bookmarks() {
  ...
  useEffect(() => {
    ...
    const storageHandler = (e: StorageEvent) => {
      if (e.key === "sunshine-bookmarks") reload();
    };
            ...
            >
              <a
                ...
                class="bookmark-link"
                  onClick={(e) => {
                  e.preventDefault();
                  (window.sunshineApp as any)?.handleSearch(bookmark.url);
                }}
              >
