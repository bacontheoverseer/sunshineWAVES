...
export default function SettingsMenu() {
  ...
  useEffect(() => {
    ...
    if (!w.__sunshineStuffData) {
      w.__sunshineStuffData = fetch("/api/stuff", { cache: "no-store" })
        .then((res) => (res.ok ? res.json() : null))
        .catch(() => null);
    }
    w.__sunshineStuffData.then((data: any) => {
  ...
  const handleExport = async () => {
    if (typeof window.sunshineExportAllData === "function") {
      try {
        ...
        a.href = url;
        a.download = `sunshine-data-${ts}.json`;
    ...
    input.onchange = (e: Event) => {
      ...
      reader.onload = async (ev: ProgressEvent<FileReader>) => {
        try {
          ...
          if (typeof (window as any).sunshineImportDataFromObject === "function") {
            await (window as any).sunshineImportDataFromObject(data);
  ...
  const allGameSources = ["selenite", "gn-math", "edurocks", "velara", "truffled"];
  const allThemes = [
    "default",
    "catppuccin",
    "nord",
          ...
          <div class={`tab-content${activeTab === "about" ? " active" : ""}`}>
            ...
            <div class="settings-item">
              ...
              <p>
                ...
                <a
                  href="https://github.com/l4uy/sunshine"
                  target="_blank"
                  class="hover-link"
                >
                  github repository
