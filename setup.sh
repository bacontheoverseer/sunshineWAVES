...
if command -v pm2 >/dev/null 2>&1; then
    ...
    pm2 stop sunshine 2>/dev/null || true
    ...
    pm2 delete sunshine 2>/dev/null || true
...
if [ -f /etc/sysctl.d/99-sunshine-optimizations.conf ]; then
  sudo cp /etc/sysctl.d/99-sunshine-optimizations.conf /etc/sysctl.d/99-sunshine-optimizations.conf.bak.$(date +%s)
fi

cat <<EOF | sudo tee /etc/sysctl.d/99-sunshine-optimizations.conf
...
EOF
sudo sysctl -p /etc/sysctl.d/99-sunshine-optimizations.conf || true
...
cat <<MOCHISVC | sudo tee /etc/systemd/system/mochi.service > /dev/null
[Unit]
Description=mochi proxy
After=network-online.target
Wants=network-online.target
...
sudo systemctl daemon-reload
sudo systemctl enable mochi >/dev/null 2>&1 || true
sudo systemctl restart mochi 2>/dev/null || true
...
SUNSHINE_MEM="$(calc_mem 16 256 2560)M"
MOCHI_MEM="$(calc_mem 24 384 5120)M"
CLOUDSYNC_MEM="$(calc_mem 6 128 768)M"
NURU_MEM="$(calc_mem 22 512 4608)M"
log "memory limits — sunshine: $SUNSHINE_MEM, mochi: $MOCHI_MEM, cloudsync: $CLOUDSYNC_MEM, nuru: $NURU_MEM"
...
module.exports = {
  apps: [
    ...
    },
    {
      ...
      min_uptime: 15000,
      kill_timeout: 10000,
      max_memory_restart: "$SUNSHINE_MEM",
      env: {
        NODE_ENV: "production",
        PORT: "3000",
...
if [ "$WG_ENABLED" -eq 0 ]; then
    PM2_SVCS="ask|sunshine|mochi|cloudsync|nuru"
else
    PM2_SVCS="ask|sunshine|mochi|cloudsync"
fi
...
log "all done! your sunshine instance is now all setup and ready to be used!!!!"